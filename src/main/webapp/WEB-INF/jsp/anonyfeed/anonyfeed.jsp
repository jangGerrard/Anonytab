...
function dialClick(no){
	...
	function commentInsert(no) {
		...
		$.get('${ctx}/main/comment/insert.do?no=' + no + '&contents='
				+ contents + '&limit=' + page+'&tag='+email+'&level='+2,
	              function(data) {
	...
	function scrollShow() {
		$.get('${ctx}/main/newsfeed/json.do?limit=' + page+'&level='+2, function(data) {
			page += 10;
			show(data);
							 ...
							 el+='<div class="panel-heading">';
							  el+='<div class="btn-group btn-group-justified">';
							    ...
							    el+='<div class="col-md-7">';
							      ...
							      el+='<img id= "profileImg" src="${ctx }/main/mypage/img/download.do" class= "img-rounded">';
						
							    el+='</div>';
							
							el+='<div class="col-md-5">';
							  var myDate=jQuery.timeago(article.createDate);//article.createDate
	    					    el +='<class="well well-sm"/>'+'<nobr><p> A N O N Y M O U S</p></nobr>';
...
// 							!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!댓글부분이다
							...
							$.each(article.comments, function(j, comment) {
								...
								el += '<img id= "minProfileImg" src="${ctx }/main/mypage/img/download.do" class= "img-rounded"><p class="commentP"> ['
										+ 'Anonymous'
										+ '] : ' + comment.contents + '<p>';
								commentCount=(j+1);
							})
...
// 							!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!댓글부분이다 여기까지							
	...
	function anoymonusNewfeedRank()
	{
		

		
		$.get('${ctx}/anonyfeed/topfivelist.do',
				function(data){
			var $topFiveDiv = $('div#topFiveOnNewsFeed');
								 ...
								 el+='<div class="panel-heading">';
								  el+='<div class="btn-group btn-group-justified">';
								    ...
								    el+='<div class="col-md-7">';
								      ...
								      el+='Anonymous Top5 Recommended Article';
							
								    el+='</div>';
								
								el+='<div class="col-md-5">';
								  
		    					    el +='<class="well well-sm" onclick=""/>  ';
		    					    el+='&nbsp; &nbsp; &nbsp;';
								    el += '<class="well well-sm" onclick=""/>Anonymous';
...
//	          			    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!Top Five Table!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
										...
										el+='<div class="panel-body">';
										el+='<div><table class="table table-striped">';
										el += '<thead><tr><th>Contents</th><th>Recommend</th><th>Create Date</th></tr></thead>';
										el += '<tbody>';
										for(var i = 0 ; i < articles.length ; i ++){
										var myDate=jQuery.timeago(articles[i].createDate);//article.createDate
											el += '<tr>';
											el += '<td>'+articles[i].contents+'</td>';
											el += '<td>'+articles[i].recommend+'</td>';
											el += '<td>'+myDate+'</td>'
											el += '</tr>';
										}
										el += '</tbody></table></div>'
...
//				          			 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!Top Five Table!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!											
	...
	$(document).ready(function() {
		...
		$.ajax({
			url : '${ctx}/main/newsfeed/json.do?level='+2,
			dataType : 'json',
									...
									<div class="panel panel-default">
										<form class="form-horizontal" id ="contentSubmit" onsubmit="onContentSubmit(); return false;"
											...
											<div class="panel-body">
												...
												<select class="btn btn-default btn-sm dropdown-toggle" id="articleLevel" name="articleLevel">
